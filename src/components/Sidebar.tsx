...
const Sidebar = () => {
  ...
  useEffect(() => {
    const checkDevice = () => {
      // True desktop detection: Needs wide screen AND fine pointer (mouse)
      const isWide = window.innerWidth >= 1024;
      const hasFinePointer = window.matchMedia('(pointer: fine)').matches;
      const isTouchOnly = window.matchMedia('(pointer: coarse)').matches;
      
      // If it's a touch device (phone/tablet), we FORCE mobile view regardless of width
      // Unless it's a very large screen with a mouse (like a desktop monitor)
