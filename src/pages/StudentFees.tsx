...
import { supabase } from '../supabaseClient';
import { toast } from 'sonner';
import { Wallet, ChevronLeft, Send, Printer, CheckCircle2, AlertCircle } from 'lucide-react';
...
const StudentFees = () => {
  ...
  const handleWhatsApp = (fee: any) => {
    const message = `*FEE INVOICE - Adarsh Shishu Mandir*%0A%0A*Student:* ${studentData.full_name}%0A*Month:* ${fee.month}%0A*Total Amount:* ₹${fee.total_amount}%0A*Status:* ${fee.status}%0A%0APlease pay via UPI or School Cash Counter.`;
    window.open(`https://wa.me/91${studentData.contact_number}?text=${message}`, '_blank');
  };

  if (loading) return <div className="h-screen flex items-center justify-center font-black uppercase text-indigo-600 italic">Syncing Ledgers...</div>;
    ...
    <div className="min-h-screen bg-[#f8fafc] p-4 md:p-10 font-sans">
      ...
      <div className="max-w-4xl mx-auto space-y-10">
        <div className="bg-indigo-900 rounded-[3.5rem] p-10 md:p-16 text-white shadow-2xl relative overflow-hidden">
           <div className="relative z-10 flex flex-col md:flex-row justify-between items-center gap-10">
              <div className="text-center md:text-left">
                 <h1 className="text-4xl md:text-6xl font-black uppercase italic tracking-tighter leading-none">Financial<br/>Summary</h1>
                 <p className="text-indigo-300 font-bold uppercase text-[10px] tracking-[0.3em] mt-6">Active Portal Account: {studentData.full_name}</p>
              </div>
              <div className="bg-white/10 backdrop-blur-xl p-10 rounded-[3rem] border border-white/10 text-center min-w-[240px] shadow-inner">
                 <p className="text-[10px] font-black uppercase text-indigo-200 mb-2 tracking-widest italic">Outstanding Dues</p>
        ...
        <div className="space-y-6">
           {fees.map(fee => (
             <div key={fee.id} className="bg-white rounded-[3rem] shadow-xl border border-gray-100 overflow-hidden group hover:border-indigo-200 transition-all">
                   ...
                   <div className="flex-1 p-10">
                         ...
                         {fee.fee_breakdown && Object.entries(fee.fee_breakdown).map(([key, val]: any) => (
                           Number(val) > 0 && (
                             <div key={key} className="flex justify-between items-center border-b border-gray-50 pb-2">
                                ...
                                <span className="font-black text-gray-900 italic">₹{val}</span>
                             </div>
                           )
                         ))}
                      </div>
                      <div className="flex flex-col md:flex-row justify-between items-center pt-6 border-t-2 border-dashed border-gray-100 gap-6">
                         ...
                         <div className="flex gap-3 w-full md:w-auto">
                            <button onClick={() => handleWhatsApp(fee)} className="flex-1 bg-emerald-500 text-white px-6 py-4 rounded-2xl font-black text-[10px] uppercase shadow-lg shadow-emerald-100 hover:bg-black transition-all flex items-center justify-center gap-2"><Send size={14}/> Share</button>
                            <button onClick={() => window.print()} className="flex-1 bg-gray-900 text-white px-6 py-4 rounded-2xl font-black text-[10px] uppercase shadow-lg hover:bg-indigo-600 transition-all flex items-center justify-center gap-2"><Printer size={14}/> Receipt</button>
                         </div>
                      </div>
                   </div>
                </div>
           ...
           {fees.length === 0 && (
             <div className="bg-white p-20 rounded-[3rem] border-4 border-dashed border-gray-100 text-center">
                ...
                <p className="text-gray-400 font-black uppercase tracking-widest italic">No fee records found in your account.</p>
             </div>
           )}
        </div>
      </div>
