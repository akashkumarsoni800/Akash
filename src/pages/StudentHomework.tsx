...
const StudentHomework = () => {
  ...
  if (loading) return (
    <div className="h-screen flex flex-col items-center justify-center bg-slate-50">
       ...
       <p className="font-black uppercase tracking-[0.4em] text-slate-400 italic text-[10px] mt-8 text-center">Syncing Assignment Manifest...</p>
    </div>
  );
      ...
      <div className="max-w-6xl mx-auto space-y-12">
        ...
        <div className="flex justify-between items-center">
          ...
          <div className="bg-slate-900 px-6 py-3 rounded-2xl border border-slate-800 shadow-xl flex items-center gap-4 group">
             ...
             <span className="text-[10px] font-black uppercase tracking-[0.2em] text-purple-400  italic">Digital Classroom Active</span>
          </div>
        </div>
        ...
        <div className="flex flex-col md:flex-row justify-between items-center md:items-end gap-10">
           ...
           <div className="bg-white border border-slate-100 rounded-[2.5rem] p-6 shadow-sm flex items-center gap-8 group hover:shadow-xl transition-all">
             ...
             <div>
               <p className="text-[9px] font-black text-slate-400 uppercase tracking-[0.2em] mb-1">Task Density</p>
               <p className="text-3xl font-black text-slate-900 tracking-tighter italic ">{homeworks.length} Assignments</p>
             </div>
           </div>
        </div>
          ...
          <AnimatePresence>
            {homeworks.length > 0 ? homeworks.map((hw, idx) => (
              ...
              >
                 <div className="flex flex-col lg:flex-row">
                    ...
                    <div className={`p-12 lg:w-96 flex flex-col justify-center items-center text-center relative transition-colors duration-500 ${
                       ...
                       <p className="text-[10px] font-black uppercase tracking-[0.3em] ">{hw.subject || 'Core Discipline'}</p>
                       <h3 className="text-3xl font-black italic uppercase tracking-tighter mt-2 ">{hw.isSubmitted ? 'Authenticated' : 'Pending'}</h3>
                       {hw.submissionDate && (
                         <div className="mt-4 flex items-center gap-2 bg-emerald-100/50 px-4 py-1.5 rounded-full">
                            ...
                            <p className="text-[9px] font-black text-emerald-700 uppercase tracking-widest">{new Date(hw.submissionDate).toLocaleDateString()} Logged</p>
                         </div>
                       )}
                    </div>
                    ...
                    <div className="flex-1 p-12 md:p-16 relative overflow-hidden flex flex-col justify-between">
                       ...
                       <div className="relative z-10 space-y-8">
                          <div className="space-y-4">
                             <h2 className="text-3xl md:text-5xl font-black text-slate-900 uppercase italic tracking-tighter leading-none ">{hw.title}</h2>
                          ...
                          <p className="text-slate-600 font-medium text-lg leading-relaxed max-w-2xl border-l-[6px] border-slate-50 pl-8 py-2 italic group-hover:border-purple-100 transition-colors">
                       ...
                       <div className="mt-12 flex flex-col sm:flex-row gap-6 items-center relative z-10">
                          <button className="w-full sm:w-auto bg-slate-900 text-white px-10 py-5 rounded-2xl font-black text-[10px] uppercase tracking-[0.2em] shadow-xl shadow-slate-200 hover:bg-purple-600 transition-all flex items-center justify-center gap-3 active:scale-95 group/btn">
                             <FileText size={18} className="group-hover/btn:rotate-12 transition-transform"/> Access Materials
                          </button>
                          {!hw.isSubmitted && (
                            <button className="w-full sm:w-auto bg-purple-600 text-white px-12 py-5 rounded-2xl font-black text-[10px] uppercase tracking-[0.2em] shadow-xl shadow-purple-100 hover:bg-slate-900 transition-all flex items-center justify-center gap-3 active:scale-95 group/submit">
              ...
              >
                 ...
                 <div className="space-y-4">
                    <h3 className="text-3xl font-black text-slate-900 uppercase tracking-tighter italic ">Repository Clear</h3>
