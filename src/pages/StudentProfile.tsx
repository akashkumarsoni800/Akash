...
const StudentProfile = () => {
  ...
  if (loading && !student) return (
     <div className="min-h-screen bg-slate-50 flex flex-col items-center justify-center">
        ...
        <p className="font-black uppercase tracking-[0.4em] text-slate-400 italic text-[10px] mt-8 text-center px-10">Synchronizing Individual Registry...</p>
     </div>
  );
  ...
  if (error || !student) return (
    <div className="min-h-screen bg-slate-50 flex flex-col items-center justify-center p-12 text-center">
      ...
      <h1 className="text-5xl font-black text-slate-900 uppercase tracking-tighter italic ">Registry Error</h1>
      <p className="text-slate-400 mt-4 font-bold uppercase tracking-widest text-xs italic">{error || "Reference node not found"}</p>
      <div className="flex gap-6 mt-12">
        <button onClick={() => navigate(-1)} className="bg-white border border-slate-200 text-slate-900 px-10 py-5 rounded-[2rem] font-black italic uppercase text-[10px] tracking-widest shadow-sm hover:shadow-xl transition-all ">Go Back</button>
        <button onClick={() => navigate("/admin/dashboard")} className="bg-slate-900 text-white px-10 py-5 rounded-[2rem] font-black italic uppercase text-[10px] tracking-widest shadow-2xl hover:bg-indigo-600 transition-all ">Dashboard</button>
      </div>
    </div>
  );
      ...
      <div className="max-w-7xl mx-auto space-y-12">
        ...
        <div className="flex justify-between items-center no-print">
          <div className="flex gap-4">
            <button onClick={() => navigate(-1)} className="flex items-center gap-4 bg-white border border-slate-100 px-8 py-5 rounded-[2rem] font-black text-[10px] uppercase text-slate-900 shadow-sm hover:shadow-xl transition-all tracking-[0.2em]  italic">
              <ChevronLeft size={18} /> Back
            </button>
            <button onClick={() => window.print()} className="bg-slate-900 text-white px-8 py-5 rounded-[2rem] font-black text-[10px] uppercase shadow-2xl flex items-center gap-4 tracking-[0.2em] hover:bg-indigo-600 transition-all  italic">
          ...
          <div className="bg-white px-6 py-3 rounded-full border border-slate-100 shadow-sm flex items-center gap-3">
             ...
             <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest italic ">Registry Live</p>
          </div>
        </div>
        ...
        >
          ...
          <div className="relative z-10 flex flex-col md:flex-row justify-between items-center gap-12">
            <div className="flex flex-col md:flex-row items-center gap-12 text-center md:text-left">
              <label className="relative group w-48 h-48 rounded-[3.5rem] bg-white/5 border-[6px] border-white/10 overflow-hidden flex items-center justify-center backdrop-blur-md cursor-pointer hover:border-indigo-400/50 hover:shadow-[0_0_50px_rgba(129,140,248,0.3)] transition-all flex-shrink-0">
                ...
                <div className="absolute inset-0 bg-indigo-900/60 flex flex-col items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity backdrop-blur-sm">
                  ...
                  <span className="text-white text-[9px] font-black uppercase tracking-[0.3em] mt-3">Sync Biometric</span>
                </div>
              ...
              <div className="space-y-6">
                <div className="inline-flex items-center gap-3 bg-emerald-500/20 text-emerald-400 px-6 py-2 rounded-full border border-emerald-500/30 shadow-lg backdrop-blur-md">
                   <ShieldCheck size={14} />
                   <span className="text-[10px] font-black uppercase tracking-[0.2em]  italic">Verified Academic Node</span>
                </div>
                <h1 className="text-5xl md:text-8xl font-black uppercase italic tracking-tighter leading-none ">{student.full_name}</h1>
                <div className="flex flex-wrap justify-center md:justify-start gap-5">
                   <div className="bg-white/5 px-6 py-2.5 rounded-2xl text-[11px] font-black border border-white/5 tracking-[0.2em] uppercase italic  flex items-center gap-3"><BookOpen size={14} className="text-indigo-400" /> Class: {student.class_name}</div>
                   <div className="bg-white/5 px-6 py-2.5 rounded-2xl text-[11px] font-black border border-white/5 tracking-[0.2em] uppercase italic  flex items-center gap-3"><GraduationCap size={14} className="text-indigo-400" /> Roll: {student.roll_no}</div>
                </div>
              </div>
            </div>
            ...
            <div className="bg-white/5 backdrop-blur-2xl p-10 rounded-[3.5rem] border border-white/10 text-center min-w-[280px] shadow-2xl relative overflow-hidden group/stat transition-all hover:bg-white/10">
              ...
              <p className="text-[11px] font-black uppercase text-indigo-300 mb-4 tracking-[0.4em] italic  relative z-10 leading-none">Attendance Rate</p>
              <p className={`text-8xl font-black tracking-tighter relative z-10 leading-none italic  transition-colors duration-500 ${attendanceRate >= 75 ? 'text-emerald-400 group-hover/stat:text-emerald-300' : 'text-rose-400 group-hover/stat:text-rose-300'}`}>
                {attendanceRate}%
              </p>
              <div className="mt-8 flex items-center justify-center gap-3 relative z-10">
                 <CalendarDays size={14} className="text-indigo-400" />
                 <p className="text-[9px] font-black text-indigo-300/40 uppercase tracking-widest italic ">Sequential Lifecycle Sync</p>
              </div>
            </div>
          </div>
        </motion.div>
          ...
          <div className="lg:col-span-2 space-y-10">
            ...
            >
              ...
              <div className="flex items-center gap-5 relative z-10 border-b border-slate-50 pb-10">
                 ...
                 <h3 className="font-black uppercase text-[12px] text-slate-900 tracking-[0.3em]  italic">Student Dossier</h3>
              </div>
              ...
              <div className="pt-8 border-t border-slate-50 opacity-30 flex items-center justify-center gap-3">
                 <ShieldCheck size={14} />
                 <p className="text-[8px] font-black uppercase tracking-[0.3em] italic ">End-to-End Encrypted Registry</p>
              </div>
            </motion.div>
            ...
            >
               <div className="p-10 border-b border-slate-50 bg-slate-50/20 flex items-center justify-between">
                  <div className="flex items-center gap-4">
                     ...
                     <h3 className="font-black uppercase text-[10px] tracking-[0.3em] text-slate-900 italic ">Identity Card</h3>
                  </div>
                  <div className="bg-slate-900 text-white px-4 py-1.5 rounded-full text-[8px] font-black uppercase tracking-widest italic ">ASM-001-ST</div>
               </div>
               ...
               <div className="px-10 py-6 bg-white border-t border-slate-50 flex justify-center">
                  <button className="text-[9px] font-black uppercase text-indigo-600 tracking-[0.4em] italic hover:text-indigo-900 transition-colors">Recalibrate Visual Identity →</button>
               </div>
            </motion.div>
          </div>
              ...
              <div className="p-12 border-b border-slate-50 bg-slate-50/20 flex items-center justify-between">
                 <div className="flex items-center gap-5">
                    ...
                    <div className="space-y-1">
                       <h3 className="font-black uppercase text-[14px] tracking-tighter text-slate-900 italic  leading-none">Financial<br/><span className="text-indigo-600">Summary</span></h3>
                       <p className="text-[8px] font-black text-slate-400 uppercase tracking-widest leading-none">Real-time Fiscal Indexing</p>
                    </div>
                 </div>
                 <div className="flex gap-4">
              ...
              <div className="p-12 space-y-12">
                ...
                <div className="space-y-6">
                  <div className="flex items-center justify-between px-6 border-b border-slate-50 pb-4">
                     <p className="text-[10px] font-black text-slate-300 uppercase tracking-[0.3em]  italic">Sequential Billing Feed</p>
                  </div>
                  ...
                  <div className="space-y-4">
                    {fees.length === 0 ? (
                      <div className="py-24 text-center opacity-10">
                        ...
                        <p className="font-black uppercase text-xs tracking-[0.5em] italic ">No Ledger Entries Recorded</p>
                      </div>
                    ) : (
                      fees.map((fee: any) => (
                        <div key={fee.id} className="bg-slate-50/50 hover:bg-white transition-all rounded-[2rem] p-8 flex flex-col md:flex-row md:items-center justify-between gap-6 border border-transparent hover:border-indigo-100 hover:shadow-xl group/row">
                           <div className="flex items-center gap-6">
                              <div className="w-12 h-12 bg-white rounded-2xl flex items-center justify-center text-slate-300 group-hover/row:text-indigo-600 border border-slate-100 shadow-sm transition-colors uppercase font-black text-xs ">
                                 {fee.month ? fee.month.charAt(0) : 'C'}
                              </div>
                              <div>
                                 <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest italic mb-1">Billing Cycle</p>
                                 <h4 className="font-black text-slate-900 uppercase text-lg italic  tracking-tighter">{fee.month || "Current Cycle"}</h4>
                              </div>
                           </div>
                           <div className="flex items-center gap-12">
                              <div className="text-right">
                                 <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest italic mb-1">Impact</p>
                                 <p className="font-black text-indigo-600 text-2xl  tracking-tighter leading-none italic">₹{Number(fee.total_amount).toLocaleString()}</p>
                              </div>
                              <span className={`px-6 py-2.5 rounded-2xl text-[10px] font-black uppercase tracking-[0.15em] shadow-sm  italic border transition-all ${
...
const DossierItem = ({ icon: Icon, label, value }: any) => (
  <div className="flex items-start gap-6 group/item">
    ...
    <div className="space-y-1">
      <p className="text-[9px] font-black text-slate-300 uppercase tracking-[0.3em]  italic group-hover/item:text-indigo-400 transition-colors leading-none">{label}</p>
      <p className="font-black text-slate-900 text-md tracking-tight  uppercase italic">{value || "Restricted Info"}</p>
    </div>
  </div>
);
...
const MiniStatCard = ({ label, value, icon: Icon, color }: any) => {
   ...
   return (
      <div className={`p-8 rounded-[2.5rem] border ${colorMap[color as keyof typeof colorMap]} shadow-inner space-y-4 group hover:shadow-xl transition-all duration-500`}>
         ...
         <div className="space-y-1">
            <p className="text-[9px] font-black uppercase tracking-[0.3em] italic  opacity-40 leading-none">{label}</p>
            <p className="text-2xl font-black  tracking-tighter leading-none italic uppercase">{value}</p>
         </div>
      </div>
   );
};
