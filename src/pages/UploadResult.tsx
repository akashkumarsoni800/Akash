...
const UploadResult = () => {
  ...
  const fetchInitialData = async () => {
    try {
      ...
      if (stdData) {
        ...
        // Create unique class list for filter
        setClasses(['All', ...new Set(stdData.map(s => s.class_name))]);
      }

      // 2. Fetch Exams (Dynamic)
  ...
  // ✅ CRASH FIX & DYNAMIC SUBJECT FILL
  const handleExamChange = (examId) => {
    const exam = exams.find(e => e.id === examId);
    setSelectedExam(examId);
    
    // Check if exam exists AND has subjects array to prevent crash
    if (exam && Array.isArray(exam.subjects)) {
      ...
      // ✅ FIX: Use 'exam.subjects.length' instead of 'sub.length'
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const { error } = await supabase.from('results').insert({
        ...
        exam_id: selectedExam, // Storing Exam ID for dynamic linking
        marks_data: results,   // Storing JSON data
    ...
    <div className="min-h-screen bg-[#F8FAFC] p-4 md:p-8">
      <div className="max-w-6xl mx-auto">
        ...
        <div className="flex items-center gap-4 mb-8">
          ...
          <h1 className="text-3xl font-black text-gray-900 tracking-tighter uppercase">Result Portal</h1>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
          ...
          <div className="lg:col-span-4 space-y-4">
            <div className="bg-white p-6 rounded-[2.5rem] shadow-sm border border-gray-100">
              <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-4 italic">Step 1: Select Candidate</p>
              ...
              <div className="space-y-3 mb-6">
                ...
                <div className="relative">
                  ...
                  <ChevronDown className="absolute right-3 top-1/2 -translate-y-1/2 text-blue-900 pointer-events-none" size={14} />
                </div>
              </div>
            ...
            <form onSubmit={handleSubmit} className="bg-white p-8 rounded-[2.5rem] shadow-sm border border-gray-100">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
                <div className="space-y-2">
                  <label className="text-[10px] font-black text-blue-900 uppercase tracking-widest ml-2 italic">Step 2: Choose Exam (Auto-Fill)</label>
                  <div className="relative">
                    <select 
                      required
                      className="w-full bg-blue-50 border-2 border-blue-100 rounded-2xl px-5 py-4 font-black text-blue-900 appearance-none focus:ring-4 focus:ring-blue-100 transition-all cursor-pointer"
                      style={{ color: '#1e3a8a' }} // ✅ VISIBILITY FIX: Forces Dark Blue Text
                      value={selectedExam}
                      onChange={(e) => handleExamChange(e.target.value)}
                    >
                      <option value="" className="text-gray-400">--- SELECT EXAM ---</option>
                      {exams.map(ex => (
                        <option key={ex.id} value={ex.id} style={{ color: '#1e3a8a' }}>{ex.exam_name.toUpperCase()}</option>
                      ))}
                    </select>
                    <ChevronDown className="absolute right-5 top-1/2 -translate-y-1/2 text-blue-900 pointer-events-none" size={18} />
                  </div>
                </div>
                ...
                {selectedStudent && (
                  <div className="bg-emerald-50 p-4 rounded-2xl flex items-center gap-4 border border-emerald-100 self-end animate-in fade-in zoom-in">
                    <div className="bg-emerald-600 text-white p-2 rounded-xl shadow-lg"><UserCircle size={20}/></div>
                    <div>
                      <p className="text-[10px] font-black text-emerald-900 uppercase leading-none">{selectedStudent.full_name}</p>
                      <p className="text-[8px] font-bold text-emerald-500 mt-1 uppercase">Ready to Grade</p>
                    </div>
                  </div>
                )}
              </div>
                  ...
                  {results.map((res, idx) => (
                    <div key={idx} className="flex gap-3 items-center group animate-in slide-in-from-right-2">
                      ...
                      <input 
                        ...
                        className="flex-1 bg-white border-none rounded-xl px-4 py-3 text-xs font-black text-gray-800 shadow-sm focus:ring-2 focus:ring-blue-900 uppercase"
                        onChange={(e) => {
              ...
              >
                {loading ? 'SYNCING DATA...' : 'CONFIRM & PUBLISH'}
              </button>
            </form>
          </div>
        </div>
