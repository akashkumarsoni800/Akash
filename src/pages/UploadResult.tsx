...
const UploadResult = () => {
  ...
  // ✅ FIXED: Dynamic Subject Auto-Fill Logic
  const handleExamChange = (examId) => {
    const exam = exams.find(e => e.id === examId);
    setSelectedExam(examId);
    
    // Safety Check: Agar exam exist karta hai aur usme subjects array hai
    if (exam && Array.isArray(exam.subjects)) {
      ...
      // Fixed: Pehle yahan 'sub.length' likha tha jo crash kar raha tha
      toast.success(`${exam.subjects.length} Subjects Loaded Successfully!`);
    } else {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const { error } = await supabase.from('results').insert({
        ...
        exam_id: selectedExam, // Link result to the exam
        marks_data: results,
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
              ...
              <div className="max-h-[400px] overflow-y-auto space-y-2 pr-2 custom-scrollbar">
                {filteredStudents.map(s => (
                  <button 
                    ...
                    className={`w-full flex items-center gap-3 p-3 rounded-2xl transition-all ${selectedStudent?.id === s.id ? 'bg-blue-900 text-white shadow-lg shadow-blue-200' : 'bg-gray-50 hover:bg-gray-100'}`}
                  >
          ...
          <div className="lg:col-span-8">
            <form onSubmit={handleSubmit} className="bg-white p-8 rounded-[2.5rem] shadow-sm border border-gray-100">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
                ...
                <div className="space-y-2">
                  ...
                  <div className="relative">
                    <select 
                      required
                      className="w-full bg-blue-50 border-2 border-blue-100 rounded-2xl px-5 py-4 mt-1 font-black text-blue-900 appearance-none focus:ring-4 focus:ring-blue-100 transition-all cursor-pointer"
                      style={{ color: '#1e3a8a' }} // Forced Color Fix
                      value={selectedExam}
                ...
                <div className="bg-gray-50/50 p-4 rounded-3xl space-y-3 border border-gray-100">
                  {results.map((res, idx) => (
                    <div key={idx} className="flex gap-3 items-center group animate-in slide-in-from-right-2">
                      <div className="bg-white p-3 rounded-xl text-gray-400 group-hover:text-blue-900 transition-colors shadow-sm"><BookOpen size={16}/></div>
                      <input 
              ...
              >
                {loading ? 'PUBLISHING RESULT...' : 'CONFIRM & UPLOAD'}
              </button>
            </form>
          </div>
        </div>
