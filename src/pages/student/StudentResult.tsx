...
const StudentResult = () => {
 ...
 if (isLoading && !student) return (
  <div className="h-screen flex flex-col items-center justify-center bg-slate-50">
    ...
    <p className="font-black  text-slate-400 text-[10px] mt-8 uppercase">Syncing Academic Ledger...</p>
  </div>
 );
   ...
   <div className="max-w-full mx-auto space-y-12">
    ...
    <div className="flex justify-between items-center">
     ...
     <div className="bg-slate-900 px-6 py-3 rounded-[5px] border border-slate-800 shadow-2xl active:scale-95 tracking-widest flex items-center gap-4 group">
       ...
       <span className="text-[10px] font-black  text-emerald-400 uppercase">Academic Sync Active</span>
     </div>
    </div>
    ...
    <div className="flex flex-col md:flex-row justify-between items-center md:items-end gap-10">
      <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} className="">
       <h1 className="text-5xl md:text-7xl font-black text-slate-900  leading-none uppercase">
        Academic<br/>
        <span className="text-emerald-600">Ledger</span>
       </h1>
      ...
      <div className="bg-white border border-slate-100 rounded-[5px] p-6 shadow-sm flex items-center gap-8 group hover:shadow-2xl active:scale-95 tracking-widest transition-all">
       ...
       <div>
        <p className="text-[9px] font-black text-slate-400  mb-1 uppercase">Scholastic Index</p>
        <p className="text-3xl font-black text-slate-900 uppercase">{results.length} Certifications</p>
       </div>
      </div>
    </div>
     ...
     <AnimatePresence>
      {results.length > 0 ? results.map((res: any, idx: number) => (
       ...
       >
         <div className="flex flex-col lg:flex-row">
          ...
          <div className="p-12 lg:w-96 bg-emerald-50 flex flex-col justify-center items-center text-center relative overflow-hidden group-hover:bg-emerald-600 transition-colors duration-700">
            ...
            <p className="text-[10px] font-black text-emerald-600 group-hover:text-emerald-100 transition-colors uppercase truncate max-w-[200px]">{res.exams?.exam_name || 'Terminal Exam'}</p>
            <h3 className="text-4xl font-black text-slate-900 group-hover:text-white mt-2 transition-colors uppercase">PROMOTED</h3>
          </div>
          ...
          <div className="flex-1 p-12 md:p-20 relative overflow-hidden">
            ...
            <div className="relative z-10 space-y-12">
             <div className="space-y-4">
               <h2 className="text-4xl md:text-6xl font-black text-slate-900  leading-none uppercase">{res.exams?.subject || 'Subject Profile'}</h2>
               <div className="flex items-center gap-4 text-[11px] font-black  text-slate-400">
                <Target size={16} className="text-emerald-600"/> 
                <span className="bg-slate-100 px-5 py-2 rounded-[5px] uppercase">Marks Secured: {res.marks_obtained} / {res.exams?.total_marks || '100'}</span>
               </div>
             </div>

             <div className="grid grid-cols-2 md:grid-cols-4 gap-8">
               <div className="space-y-2">
                 <p className="text-[10px] font-black text-slate-300 uppercase tracking-widest">Aggregate Score</p>
                 <p className="text-2xl font-black text-slate-900 uppercase">{Math.round((res.marks_obtained / (res.exams?.total_marks || 100)) * 100)}%</p>
               </div>
               <div className="space-y-2">
                 <p className="text-[10px] font-black text-slate-300 uppercase tracking-widest">Status Node</p>
                 <div className="flex items-center gap-2">
                   <div className="w-2 h-2 rounded-full bg-emerald-500 animate-pulse" />
                   <p className="text-lg font-black text-emerald-600 uppercase">AUTHENTICATED</p>
                 </div>
               </div>
               <div className="space-y-2">
                 <p className="text-[10px] font-black text-slate-300 uppercase tracking-widest">Digital Sign</p>
             ...
             <div className="pt-10 border-t border-slate-50 flex flex-col sm:flex-row gap-6">
              <button className="flex-1 bg-slate-900 text-white px-10 py-5 rounded-[5px] font-black text-[10px]  shadow-2xl active:scale-95 tracking-widest shadow-slate-200 hover:bg-emerald-600 transition-all flex items-center justify-center gap-3 active:scale-95 uppercase group/btn">
                <Download size={18} className="group-hover/btn:translate-y-1 transition-transform"/> Download Certificate
              </button>
       ...
       <div className="py-32 text-center bg-white rounded-[5px] border-4 border-dashed border-slate-100 shadow-inner group">
         ...
         <div className="space-y-4">
          <h3 className="text-3xl font-black text-slate-900 uppercase">Records Void</h3>
          <p className="max-w-md mx-auto text-slate-400 font-black text-[10px]  leading-relaxed px-10 uppercase">
           No academic performance nodes detected in your identity hash. Maintain synchronization for upcoming result emissions.
          </p>
         </div>
       </div>
      )}
