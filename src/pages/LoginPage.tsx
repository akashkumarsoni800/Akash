  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (role === 'student') {
        ...
        // ✅ TEXT BASED CHECK: 'approved' check karega
    ...
    <div className="min-h-screen flex items-center justify-center bg-gray-100 p-4">
      <div className="bg-white p-8 rounded-lg shadow-md w-full max-w-md">
        ...
        <div className="mt-4 text-center text-sm space-y-2">
           <button onClick={() => navigate('/reset-password')} className="text-blue-600 hover:underline block w-full">Forgot Password?</button>
           <div className="border-t pt-2 mt-2 text-gray-500">New Student? <button onClick={() => navigate('/profile-setup')} className="text-blue-600 font-bold hover:underline">Update Profile</button></div>
        </div>
      </div>
    </div>
  );
